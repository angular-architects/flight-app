...
function abortSignalToObservable(signal3) {
    if (signal3.aborted) {
    ...
    return new Observable(subscriber => {
        ...
        signal3.addEventListener("abort", handler);
        return () => signal3.removeEventListener("abort", handler);
    });
}
function takeUntilAbort(signal3) {
    return takeUntil(abortSignalToObservable(signal3));
}
...
function namedOutletsRedirect$1(redirectTo) {
    throw new _RuntimeError(4e3, (typeof ngDevMode === "undefined" || ngDevMode) && `Only absolute redirects can have named outlets. redirectTo: '${redirectTo}'`);
}
...
This is currently a dev mode only error but will become a call stack size exceeded error in production in a future major version.`);
    ...
    matchSegmentAgainstRoute(injector, rawSegment, route, segments, outlet, parentRoute) {
        return __async(this, null, function* () {
            ...
            const { routes: childConfig } = yield this.getChildConfig(injector, route, segments);
...
function recognize(injector, configLoader, rootComponentType, config, serializer, paramsInheritanceStrategy, abortSignal) {
    ...
    return mergeMap(t => of(t).pipe(switchMap(t2 => recognizeImpl(injector, configLoader, rootComponentType, config, t2.extractedUrl, serializer, paramsInheritanceStrategy, abortSignal)), map(({ state: targetSnapshot, tree: urlAfterRedirects }) => {
...
function provideRouter(routes, ...features) {
    ...
    return makeEnvironmentProviders2([{
            provide: ROUTES,
            multi: true,
            useValue: routes
