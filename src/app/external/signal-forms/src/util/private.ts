...
export function privateGetComponentInstance(injector: Injector): unknown {
  ...
  if (
    injector._tNode.directiveStart === 0 ||
    injector._tNode.componentOffset === -1
  ) {
    return undefined;
  }

  return injector._lView[
    injector._tNode.directiveStart + injector._tNode.componentOffset
  ];
}

export function privateSetComponentInput<T>(
...
export function privateIsSignalInput(
  value: unknown
): value is InputSignal<unknown> {
  return isInputSignal(value);
}

export function privateIsModelInput<T>(
  value: unknown
): value is ModelSignal<T> {
...
function assertIsNodeInjector(
  injector: Injector
): asserts injector is NgNodeInjector {
