...
export abstract class FieldNodeStructure {
  ...
  getChild(key: PropertyKey): FieldNode | undefined {
    ...
    if (isArray(value)) {
      ...
      if (
        isObject(childValue) &&
        childValue.hasOwnProperty(this.identitySymbol)
      ) {
    ...
    return map.get(
      (typeof key === 'number' ? key.toString() : key) as TrackingKey
    );
  }
    ...
    if (identityInParent === undefined) {
      ...
      this.keyInParent = computed(() => {
        if (parent.structure.childrenMap()?.get(key as TrackingKey) !== node) {
          throw new Error(
            `RuntimeError: orphan field, looking for property '${key}' of ${getDebugName(
              parent
            )}`
          );
        }
      ...
      this.keyInParent = computed(() => {
        ...
        if (!isArray(parentValue)) {
          ...
          throw new Error(
            `RuntimeError: orphan field, expected ${getDebugName(
              parent
            )} to be an array`
          );
        }
        ...
        throw new Error(
          `RuntimeError: orphan field, can't find element in array ${getDebugName(
            parent
          )}`
        );
      });
      ...
      if (childrenMap !== undefined) {
        ...
        if (isValueArray) {
          ...
          for (let i = 0; i < value.length; i++) {
            ...
            if (
              isObject(childValue) &&
              childValue.hasOwnProperty(identitySymbol)
            ) {
