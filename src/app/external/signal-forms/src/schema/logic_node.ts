...
export abstract class AbstractLogicNodeBuilder {
  ...
  abstract addDisabledReasonRule(
    logic: LogicFn<any, DisabledReason | undefined>
  ): void;
...
export class LogicNodeBuilder extends AbstractLogicNodeBuilder {
  ...
  override hasLogic(builder: AbstractLogicNodeBuilder): boolean {
    ...
    return this.all.some(({ builder: subBuilder }) =>
...
class NonMergeableLogicNodeBuilder extends AbstractLogicNodeBuilder {
  ...
  override addDisabledReasonRule(
    ...
    this.logic.disabledReasons.push(
      setBoundPathDepthForResolution(logic, this.depth)
    );
  }
  ...
  override addSyncErrorRule(logic: LogicFn<any, ValidationResult>): void {
    this.logic.syncErrors.push(
      setBoundPathDepthForResolution(logic, this.depth)
    );
  }

  override addSyncTreeErrorRule(logic: LogicFn<any, ValidationResult>): void {
    this.logic.syncTreeErrors.push(
      setBoundPathDepthForResolution(logic, this.depth)
    );
  }

  override addAsyncErrorRule(logic: LogicFn<any, AsyncValidationResult>): void {
    this.logic.asyncErrors.push(
      setBoundPathDepthForResolution(logic, this.depth)
    );
  }
  ...
  override addAggregatePropertyRule<T>(
    ...
    this.logic
      .getAggregateProperty(key)
      .push(setBoundPathDepthForResolution(logic, this.depth));
  }
  ...
  override addPropertyFactory<D>(
    ...
    this.logic.addPropertyFactory(
      key,
      setBoundPathDepthForResolution(factory, this.depth)
    );
  }
...
class LeafLogicNode implements LogicNode {
  ...
  constructor(
    ...
    this.logic = builder
      ? createLogic(builder, predicates, depth)
      : new LogicContainer([]);
  }
  ...
  getChild(key: PropertyKey): LogicNode {
    ...
    const childBuilders = this.builder
      ? getAllChildBuilders(this.builder, key)
      : [];
    if (childBuilders.length === 0) {
      ...
      return new LeafLogicNode(
        ...
        [
          ...this.predicates,
          ...predicates.map((p) => bindLevel(p, this.depth)),
        ],
        this.depth + 1
      );
    } else {
...
class CompositeLogicNode implements LogicNode {
  ...
  getChild(key: PropertyKey): LogicNode {
    return new CompositeLogicNode(
      this.all.flatMap((child) => child.getChild(key))
    );
  }
