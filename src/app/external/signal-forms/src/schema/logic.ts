...
> {
  ...
  static ignoreNull<TElement>(predicates: ReadonlyArray<BoundPredicate>) {
    return new ArrayMergeIgnoreLogic<TElement, null>(
      predicates,
      (e: unknown) => e === null
    );
  }
  ...
  constructor(
    ...
    private ignore:
      | undefined
      | ((e: TElement | undefined | TIgnore) => e is TIgnore)
  ) {
    super(predicates);
  }
  ...
  override compute(arg: FieldContext<any>): readonly TElement[] {
    return this.fns.reduce((prev, f) => {
      ...
      if (value === undefined || value === IGNORED) {
        ...
        return [
          ...prev,
          ...(this.ignore ? value.filter((e) => !this.ignore!(e)) : value),
        ];
      } else {
        if (
          this.ignore &&
          this.ignore(value as TElement | TIgnore | undefined)
        ) {
          return prev;
        }
        return [...prev, value];
...
/** Logic that combines its individual logic function results by aggregating them in an array. */
export class ArrayMergeLogic<TElement> extends ArrayMergeIgnoreLogic<
  TElement,
  never
> {
...
export class AggregatePropertyMergeLogic<TAcc, TItem> extends AbstractLogic<
  TAcc,
  TItem
> {
  override get defaultValue() {
...
function wrapWithPredicates<TValue, TReturn>(
  ...
  return (arg: FieldContext<any>): TReturn | typeof IGNORED => {
    for (const predicate of predicates) {
      ...
      // doesn't change.
      const depthDiff =
        untracked(predicateField.structure.pathKeys).length - predicate.depth;
...
export class LogicContainer {
  ...
  constructor(private predicates: ReadonlyArray<BoundPredicate>) {
    ...
    this.syncErrors =
      ArrayMergeIgnoreLogic.ignoreNull<ValidationError>(predicates);
    this.syncTreeErrors =
      ArrayMergeIgnoreLogic.ignoreNull<ValidationError>(predicates);
    this.asyncErrors = ArrayMergeIgnoreLogic.ignoreNull<
      ValidationError | 'pending'
    >(predicates);
  }
   ...
   */
  getAggregateProperty<T>(
    prop: AggregateProperty<unknown, T>
  ): AbstractLogic<T> {
    if (
      !this.aggregateProperties.has(prop as AggregateProperty<unknown, unknown>)
    ) {
