...
export class Control<T> {
  ...
  private initialize() {
    ...
    effect(
      (onCleanup) => {
        ...
        fieldNode.nodeState.controls.update((controls) => [
          ...controls,
          this as Control<unknown>,
        ]);
        onCleanup(() => {
          fieldNode.nodeState.controls.update((controls) =>
  ...
  private setupNativeInput(
    ...
    this.maybeSynchronize(
      this.propertySource(MIN),
      this.withAttribute(input, 'min')
    );
    this.maybeSynchronize(
      this.propertySource(MIN_LENGTH),
      this.withAttribute(input, 'minLength')
    );
    this.maybeSynchronize(
      this.propertySource(MAX),
      this.withAttribute(input, 'max')
    );
    this.maybeSynchronize(
      this.propertySource(MAX_LENGTH),
      this.withAttribute(input, 'maxLength')
    );
      ...
      case 'select':
        this.maybeSynchronize(
          ...
          (value) => {
            ...
            afterNextRender(() => (input.value = value as string), {
              injector: this.injector,
            });
          }
        );
  ...
  private setupCustomUiControl(cmp: FormUiControl) {
    ...
    this.maybeSynchronize(
      () => this.state().disabled(),
      withInput(cmp.disabled)
    );
    this.maybeSynchronize(
      () => this.state().disabledReasons(),
      withInput(cmp.disabledReasons)
    );
    this.maybeSynchronize(
      () => this.state().readonly(),
      withInput(cmp.readonly)
    );
    ...
    if (privateIsModelInput(cmp.touched) || isOutputRef(cmp.touched)) {
      ...
      cleanupDefaultTouch = () =>
        this.el.nativeElement.removeEventListener('focusout', listener);
    }
  ...
  private propertySource<T>(key: AggregateProperty<T, any>): () => T {
    const metaSource = computed(() =>
      this.state().hasProperty(key)
        ? this.state().property(key)
        : key.getInitial
    );
    return () => metaSource()?.();
  }
...
function isFormUiControl(cmp: unknown): cmp is FormUiControl {
  ...
  return (
    ...
    (castCmp.readonly === undefined ||
      privateIsSignalInput(castCmp.readonly)) &&
    (castCmp.disabled === undefined ||
      privateIsSignalInput(castCmp.disabled)) &&
    (castCmp.disabledReasons === undefined ||
      privateIsSignalInput(castCmp.disabledReasons)) &&
    ...
    (castCmp.minLength === undefined ||
      privateIsSignalInput(castCmp.minLength)) &&
...
function isFormValueControl(
  cmp: FormUiControl
): cmp is FormValueControl<unknown> {
...
function isShadowedControlComponent(cmp: unknown): boolean {
  ...
  return (
    mirror?.inputs.some((input) => input.templateName === 'control') ?? false
  );
}
