...
describe('control directive', () => {
  ...
  it('should update bound controls on the field when it is bound and unbound', async () => {
    ...
    expect(f().controls()).toEqual([
      fixture.componentInstance.controlDirective(),
    ]);
  ...
  it('should synchronize custom properties', () => {
    ...
    const comp = act(() =>
      TestBed.createComponent(CustomPropsTestCmp)
    ).componentInstance;
  ...
  it('should synchronize readonly', () => {
    @Component({
      template: ` <input #text type="text" [control]="f" /> `,
      imports: [Control],
    })
    ...
    const comp = act(() =>
      TestBed.createComponent(ReadonlyTestCmp)
    ).componentInstance;
  ...
  it('should allow binding error and disabled messages through control or manually', () => {
    ...
    class TestCmp {
      ...
      f = form(this.model, (p) => {
        ...
        disabled(p, ({ value }) =>
          value() === 'disabled' ? 'schema disabled' : false
        );
      });
    ...
    const fix = act(() => TestBed.createComponent(TestCmp));
    expect(
      [...fix.nativeElement.querySelectorAll('.error')].map(
        (e) => e.textContent
      )
    ).toEqual(['manual error', 'schema error']);
    act(() => fix.componentInstance.model.set('disabled'));
    expect(
      [...fix.nativeElement.querySelectorAll('.disabled-reason')].map(
        (e) => e.textContent
      )
...
function setupRadioGroup() {
  ...
  const fix = act(() => TestBed.createComponent(TestCmp));
  const formEl = (fix.nativeElement as HTMLElement)
