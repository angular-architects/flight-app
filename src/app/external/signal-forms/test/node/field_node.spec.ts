...
describe('FieldNode', () => {
        ...
        (p) => {
          applyEach(p.names, (a) => {
            disabled(a.name, ({ value, fieldOf }) => {
              ...
              expect(
                [...fieldOf(p).names].findIndex((e: any) => e === el)
              ).not.toBe(-1);
              return true;
            });
          });
  ...
  describe('disabled', () => {
    ...
    it('should not have disabled reason if not disabled', () => {
      const f = form(
        ...
        (p) => {
          disabled(p.a, ({ value }) =>
            value() > 5 ? 'a cannot be changed' : false
          );
        },
        { injector: TestBed.inject(Injector) }
      );
  ...
  describe('validation', () => {
      ...
      const f = form(
        ...
        (p) => {
          validate(p.a, ({ value }) => {
            if (value() > 10) {
              return [
                customError({ kind: 'too-damn-high' }),
                customError({ kind: 'bad' }),
              ];
            }
            return undefined;
          });
    ...
    describe('tree validation', () => {
      it('should push errors to children', () => {
          ...
          (p) => {
            validateTree(p, ({ value, fieldOf }) => {
              ...
              if (value().name.length > 8) {
                errors.push(
                  customError({ kind: 'long_name', field: fieldOf(p.name) })
                );
              }
              if (value().age < 0) {
                errors.push(
        ...
        expect(f.name().errors()).toEqual([]);
        expect(f.age().errors()).toEqual([
          customError({ kind: 'temporal_anomaly', field: f.age }),
        ]);

        cat.set({ name: 'Fluffy McFluffington', age: 10 });
        expect(f.name().errors()).toEqual([
          customError({ kind: 'long_name', field: f.name }),
        ]);
        expect(f.age().errors()).toEqual([]);
      });
  ...
  describe('predefined schema', () => {
    ...
    it('should error on resolving predefined schema path that is not part of the form', () => {
      ...
      expect(() => f().disabled()).toThrowError(
        'Path is not part of this field tree.'
      );
    });
  });
