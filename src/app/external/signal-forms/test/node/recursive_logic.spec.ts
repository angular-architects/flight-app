...
function narrowed<TValue, TNarrowed extends TValue>(
  ...
  return computed(
    () =>
      field &&
      (guard(field().value()) ? (field as Field<TNarrowed>) : undefined)
  );
}
...
describe('recursive schema logic', () => {
  it('should support recursive logic', () => {
    ...
    expect(
      narrowed(narrowed(f.next, isNonNull)()?.next, isNonNull)()
        ?.level()
        .disabled()
    ).toBe(true);
    expect(
      narrowed(
        narrowed(narrowed(f.next, isNonNull)()?.next, isNonNull)()?.next,
        isNonNull
      )()
        ?.level()
        .disabled()
  ...
  it('should support recursive logic with arrays', () => {
        ...
        (children) => {
          applyEach(children, (c) => {
            validate(c.tag, ({ value }) =>
              value() !== 'tr'
                ? customError({ kind: 'invalid-child' })
                : undefined
            );
          });
        }
    ...
    data.set({
      tag: 'table',
      children: [{ tag: 'tr', children: [{ tag: 'span', children: [] }] }],
    });
    expect(f().valid()).toBe(false);

    data.set({
      tag: 'table',
      children: [{ tag: 'tr', children: [{ tag: 'td', children: [] }] }],
    });
    expect(f().valid()).toBe(true);
  });
});
